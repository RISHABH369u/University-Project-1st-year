 ...
 *   initDevTool();
 *
 * REQUIRED patch in interaction.js — wrap every orbit handler:
...
// Orbit guard — call this in interaction.js every mouse handler
window._dtCanOrbit = () => !STATE.isDraggingTC;


// ═══════════════════════════════════════════════════════════════════════
// §2  HELPERS
// ★ FIX 1: camera.updateMatrixWorld(true) before EVERY raycast
//   Without this, with a custom orbit system the camera matrices can be
//   stale, so rays go to wrong world positions → walls land in wrong spots.
// ═══════════════════════════════════════════════════════════════════════
...
function _gndPt(e) {
  camera.updateMatrixWorld(true); // ← THE KEY FIX for wall drawing
...
// ★ FIX 2: Capture-phase event interception kills orbit during TC drag.
//   We register listeners in CAPTURE phase (3rd arg = true) on the canvas
//   and on window. These fire BEFORE any bubble-phase orbit handlers in
//   interaction.js. stopImmediatePropagation() prevents orbit from seeing
//   the events. TC itself uses pointer-capture so it keeps working.
// ═══════════════════════════════════════════════════════════════════════
...
// Block orbit events in capture phase while TC is active
function _captureKill(e) {
  if (STATE.isDraggingTC) e.stopImmediatePropagation();
}

// On canvas — stops interaction.js canvas listeners
...
function _onClick(e) {
  ...
  if (['select', 'translate', 'rotate', 'scale'].includes(tool)) {
    camera.updateMatrixWorld(true); // ★ also fix for object picking
